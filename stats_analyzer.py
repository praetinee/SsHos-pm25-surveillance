...
def render_statistical_matrix(df_filtered, df_pm25):
    ...
    # เพิ่มกล่องคำอธิบายระเบียบวิธีวิจัยและสถิติที่ใช้ให้ชัดเจน
    st.markdown("#### 📚 คำอธิบายระเบียบวิธีวิจัยและสถิติที่ใช้")
    st.info(
        f"**สถิติหลักที่ใช้ในการวิเคราะห์:** โมเดลสมการถดถอยควาไซ-ปัวซง (Quasi-Poisson Regression)\n\n"
        f"**1. ทำไมถึงเลือกใช้ และเหมาะกับข้อมูลอย่างไร?**\n"
        f"ข้อมูลที่นำมาวิเคราะห์คือ 'จำนวนผู้ป่วยต่อเดือน' (Count Data) ซึ่งเป็นข้อมูลนับที่เป็นจำนวนเต็มบวก โมเดลตระกูล Poisson ถูกออกแบบมาเพื่อจัดการกับข้อมูลประเภทนี้โดยตรง นอกจากนี้ข้อมูลทางระบาดวิทยาที่เกี่ยวข้องกับมลพิษมักผูกกับฤดูกาล ทำให้มีลักษณะเฉพาะคือ บางเดือนผู้ป่วยน้อยมาก และบางเดือนผู้ป่วยพุ่งสูงปรี๊ดตามสภาพอากาศ\n\n"
        f"**2. ทำไมถึงไม่ใช้ Poisson Regression ธรรมดา?**\n"
        f"Poisson ธรรมดามีข้อจำกัดสำคัญคือมีสมมติฐานว่า 'ความแปรปรวนต้องเท่ากับค่าเฉลี่ย' (Mean = Variance) แต่ข้อมูลผู้ป่วยในโลกความเป็นจริงมักเกิดภาวะ **Overdispersion** (ข้อมูลกระจายตัวและเหวี่ยงสูงกว่าค่าเฉลี่ยมาก) หากใช้ Poisson ธรรมดา โมเดลจะประเมินค่าความคลาดเคลื่อนต่ำเกินไป ส่งผลให้ค่า p-value 'มีนัยสำคัญหลอก' (False Positive) หรือแปลว่าโมเดลฟันธงว่าฝุ่นมีผล ทั้งที่ความจริงอาจเป็นแค่จังหวะข้อมูลแกว่งตัว การใช้ Quasi-Poisson จะเพิ่มพารามิเตอร์ (Dispersion parameter) มาปรับค่าช่วงความเชื่อมั่นให้กว้างและเข้มงวดขึ้นตามจริง ช่วยให้ผลลัพธ์แม่นยำและไม่หลอกตา\n\n"
        f"**3. ทำไมถึงไม่ใช้สถิติพื้นฐานอย่าง Linear Regression?**\n"
        f"Linear Regression หรือ OLS เหมาะกับข้อมูลที่มีความต่อเนื่อง มีโอกาสติดลบ และมีการแจกแจงแบบปกติ (Normal Distribution) แต่จำนวนผู้ป่วยไม่สามารถติดลบได้ และมักเบ้ขวา การใช้สถิติทั่วไปจึงบิดเบือนสมมติฐานและให้ค่าพยากรณ์ที่ผิดพลาด\n\n"
        f"**ข้อจำกัดของ Quasi-Poisson Regression:**\n"
        f"เนื่องจากโมเดลนี้อาศัยฟังก์ชันความน่าจะเป็นแบบกึ่งหนึ่ง (Quasi-likelihood) ไม่ใช่ Likelihood ที่แท้จริง จึงทำให้ไม่สามารถคำนวณค่าสถิติบางตัว เช่น AIC (Akaike Information Criterion) หรือ BIC เพื่อนำไปเปรียบเทียบหาความเหมาะสมที่สุดของโมเดล (Goodness-of-fit) เทียบกับสมการรูปแบบอื่น ๆ ได้โดยตรง"
    )
