...
def render_statistical_matrix(df_filtered, df_pm25):
    ...
    if diagnostic_data:
        ...
        st.caption("ตารางที่ 3 แสดงค่าสถิติเพื่อประเมินความเหมาะสมของสมการ Quasi-Poisson (Goodness-of-Fit) ซึ่งสามารถนำไปอ้างอิงในเอกสารวิชาการได้")
        
        df_diag = pd.DataFrame(diagnostic_data)
        st.dataframe(df_diag, use_container_width=True, hide_index=True)
        
        st.info(
            "**วิธีแปลผลตารางประเมินคุณภาพโมเดล (สำหรับนำไปเขียนในเล่ม อวช.):**\n"
            "- **Null vs Residual Deviance:** ค่า Residual Deviance ที่ลดลงจาก Null Deviance เป็นหลักฐานยืนยันว่าการนำ PM2.5, ฤดูกาล และเวลา เข้ามาในสมการ ช่วยให้อธิบายความคลาดเคลื่อนของจำนวนผู้ป่วยได้ดีขึ้นจริง\n"
            "- **Pseudo R²:** บอกสัดส่วนความแปรปรวนที่โมเดลอธิบายได้ ยิ่งเข้าใกล้ 1 ยิ่งดี (อย่างไรก็ตาม ในงานวิจัยด้านระบาดวิทยาที่จำกัดตัวแปรควบคุม การได้ค่าที่ต่ำไม่ใช่เรื่องผิดปกติ แต่ชี้ให้เห็นว่ายังมีปัจจัยอื่นนอกเหนือจากฝุ่นที่ส่งผลต่อการป่วย)\n"
            "- **Deviance Residuals:** ค่าความคลาดเคลื่อนตกค้าง (Min ถึง Max) ควรจะกระจายตัวอย่างสมดุล (ไม่เหวี่ยงไปทางลบหรือบวกมากเกินไป) หากช่วงกว้างมาก อาจสะท้อนถึงการมีผู้ป่วยกลุ่ม Outlier ในบางเดือน"
        )
        st.markdown("---")
