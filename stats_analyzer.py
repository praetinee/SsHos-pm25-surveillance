...
def render_statistical_matrix(df_filtered, df_pm25):
    ...
    st.info(
        ...
        f"**เหตุผลที่ใช้สถิตินี้เป็นมาตรฐานเดียวในระบบ:**\n"
        f"1. **ความสม่ำเสมอของการวิจัย (Consistency):** การใช้สถิติตัวเดียวคลุมทุกกลุ่มข้อมูล ช่วยให้ตารางทั้งหมดเป็นมาตรฐานเดียวกัน สามารถเปรียบเทียบผลกระทบข้ามกลุ่มโรคหรือกลุ่มอายุได้อย่างถูกต้องตามหลักวิชาการ\n"
        f"2. **จัดการการแกว่งตัวของข้อมูล (Overdispersion):** ข้อมูลจำนวนผู้ป่วยในโลกจริงมักมีการกระจายตัวสูงกว่าทฤษฎี (บางเดือนน้อยมาก บางเดือนพุ่งสูง) หากใช้ Poisson ธรรมดา อาจทำให้เกิด 'ผลบวกลวง' (สรุปว่ามีผล ทั้งที่จริงๆ แค่ข้อมูลแกว่ง) การใช้ Quasi-Poisson จะช่วยแก้ปัญหานี้ได้อย่างเด็ดขาด\n"
        f"3. **เพิ่มความน่าเชื่อถือ (Robustness):** โมเดลจะปรับช่วงความเชื่อมั่นให้เข้มงวดสอดคล้องกับความผันผวนของข้อมูล ดังนั้น หากผลลัพธ์ใดมีนัยสำคัญ (⭐ p < 0.05) จึงสามารถมั่นใจได้สูงมากว่าเป็นผลจากการเพิ่มขึ้นของฝุ่น PM2.5 จริงๆ\n\n"
        f"**💡 วิธีการอ่านผล:** ตัวเลขในตารางคือ **% จำนวนผู้ป่วยที่คาดว่าจะเปลี่ยนไป** เมื่อค่าเฉลี่ยฝุ่น PM2.5 เพิ่มขึ้นทุกๆ **{PM25_UNIT_SCALE} µg/m³**"
    )
