...
def perform_poisson_regression(df_sub, df_pm25):
    ...
    # เพิ่มการตรวจสอบ Sparse Data (ข้อมูลเบาบาง) เพื่อแก้ปัญหา Complete Separation
    non_zero_months = (merged['case_count'] > 0).sum()
    
    # 4. สร้างตัวแปรควบคุม (Control Variables)
    # 4.1 Time Trend: ควบคุมแนวโน้มระยะยาว
    merged['Time_Trend'] = np.arange(1, len(merged) + 1)
    
    # 4.2 Seasonality: สร้างตัวแปรหมวดหมู่ เป็นเดือน 1-12
    merged['Month'] = merged['Month_Year'].dt.month.astype(str)
    
    # ป้องกันสมการระเบิด ต้องมี "เดือนที่พบผู้ป่วยจริงๆ" อย่างน้อย 15 เดือน
    ...
    try:
        ...
        pseudo_r2 = 1 - (resid_dev / null_dev) if null_dev > 0 else 0
        dev_resid = model.resid_deviance
        resid_min, resid_max = dev_resid.min(), dev_resid.max()
        
        coef = model.params['PM25']
        
        # ป้องกันปัญหา Complete Separation
...
def render_statistical_matrix(df_filtered, df_pm25):
    """สร้างตารางสรุปสถิติเชิงอนุมานด้วย Quasi-Poisson"""
        ...
        for col_name, disease_name in disease_cols.items():
            ...
            if res:
                ...
                diagnostic_data.append({
                    "กลุ่มเป้าหมาย": age,
                    "กลุ่มโรค": col_name,
                    "Null Deviance": f"{res['null_dev']:.2f}",
                    "Residual Deviance": f"{res['resid_dev']:.2f}",
                    "Pseudo R²": f"{res['pseudo_r2']:.4f}",
                    "Deviance Residuals": f"[{res['resid_min']:.2f}, ถึง {res['resid_max']:.2f}]"
                })
    ...
    if diagnostic_data:
        st.markdown("#### 🔬 ตารางประเมินคุณภาพของสมการ (Model Diagnostics)")
        st.caption("ตารางที่ 3 แสดงค่าสถิติเพื่อประเมินความเหมาะสมของสมการ Quasi-Poisson ซึ่งสามารถนำไปอ้างอิงในเอกสารวิชาการได้")
        
        df_diag = pd.DataFrame(diagnostic_data)
        st.dataframe(df_diag, use_container_width=True, hide_index=True)
        
        # เพิ่มข้อมูลเรื่อง Threshold ตามที่ตกลงกันไว้
        st.info(
            "**วิธีแปลผลและเกณฑ์ประเมินคุณภาพโมเดล (Model Diagnostics) สำหรับอ้างอิงในเล่ม อวช.:**\n\n"
            "1. **Null vs Residual Deviance:** `Residual Deviance` ควรมีค่า **'ลดลง'** จาก `Null Deviance` อย่างชัดเจน การลดลงนี้เป็นหลักฐานยืนยันว่าการนำตัวแปร PM2.5, ฤดูกาล และเวลา เข้ามาในสมการ ช่วยอธิบายการเกิดโรคได้ดีกว่าการไม่ใส่ตัวแปรใดๆ เลย\n"
            "2. **Pseudo R²:** บอกสัดส่วนความแปรปรวนที่โมเดลอธิบายได้ ยิ่งเข้าใกล้ 1 ยิ่งดี ในงานวิจัยทางระบาดวิทยา การได้ค่าที่ตกอยู่ในช่วง **0.20 ถึง 0.40** (หรือมากกว่า) ถือว่าโมเดลทำงานได้ดีเยี่ยมและเป็นที่ยอมรับแล้ว เพราะการเกิดโรคมีปัจจัยอื่นแวดล้อมอีกมาก\n"
            "3. **Deviance Residuals (Min, Max):** ค่าความคลาดเคลื่อนตกค้าง ควรแกว่งตัวสมดุลรอบค่าศูนย์ โดยปกติขอบเขตไม่ควรเกิน **[-3 ถึง +3] หรืออย่างมากไม่เกิน 4** หากอยู่ในช่วงนี้แปลว่าข้อมูลกระจายตัวดี สมการเหมาะสม และไม่มีข้อมูลผิดปกติ (Outlier) ที่ทำให้สมการผิดเพี้ยน\n"
            "4. **Dispersion Parameter (ค่า Disp ในตาราง 2):** ค่าที่ **> 1.0** อย่างชัดเจน (เช่น 1.5, 2.0+) เป็นเครื่องยืนยันภาวะข้อมูลกระจายตัวสูง (Overdispersion) การที่คุณเลือกใช้ **'Quasi-Poisson'** จึงเป็นการตัดสินใจที่ถูกต้องทางสถิติที่สุด เพื่อปรับปรุงช่วงความเชื่อมั่นไม่ให้เกิดการสรุปผลนัยสำคัญหลอก (False Positive)"
        )
        st.markdown("---")
    
    # คำอธิบายระเบียบวิธีวิจัยและสถิติที่ใช้ (ปรับปรุงใหม่ตามหลักวิชาการ)
